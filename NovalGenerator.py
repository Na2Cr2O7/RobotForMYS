...
if __name__ == "__main__":
    ...
    model=torch.load('lstm.pkl',weights_only=False,map_location=torch.device(device))
    # 加载训练好的模型，并明确指定使用CPU
    #model.load_state_dict(torch.load('lstm_model.pth', map_location=torch.device('cpu')))
    model.to(device)
    generated_article=topic
    Total=''
    for _ in tqdm.tqdm(range(6)):
        generated_article = generate_article(model, corpus, num_samples=3000,start=generated_article[-1])
        Total+=generated_article
    with open('Noval.txt','w',encoding='utf8') as f:
        f.write(Total)
    print(Total)